...
// defaultConfigPath returns the default config path when CONFIG_REPO_ROOT is set,
// or empty string (flag remains required) when CONFIG_REPO_ROOT is unset.
func defaultConfigPath() string {
	if configRepoRoot() == "" {
		return ""
	}
	return resolveConfigPath("configs/talos-bootstrap.yaml")
}
